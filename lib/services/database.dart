...
class Database {
  ...
  final CollectionReference fridgeCollection = FirebaseFirestore.instance.collection('refrigerator');
  ...
  Future getProductList() async {
    ...
    String groupID = await getCurrentGroupID();
    await fridgeCollection.doc(groupID).collection("products").get().then((value){
     data = value.docs.map((doc) => doc.data()).toList();
    });
    data.forEach((element) {
      products.add(new Product(title: element["name"], number: element["number"].toDouble(), unit: element["unit"], category: element["category"]));
    });
    return products;
  }
  ...
  Future removeProductFromList(Product product) async{
    String groupID = await getCurrentGroupID();
    await fridgeCollection.doc(groupID).collection("products").doc(product.title).delete();
  }

  Future saveGoodiesPerCategoryList(String category, List<FridgeProduct> goodies)async{
    String groupID = await getCurrentGroupID();
    ...
    await fridgeCollection.doc(groupID).collection("goodies").doc(category).set({
      "goodies": fridgeProducts
    });
  }

  Future updateGoodiesPerCategoryList(String category, List<FridgeProduct> goodies)async{
    String groupID = await getCurrentGroupID();
    List<Map<String, dynamic>> fridgeProducts = [];
    for(int i = 0; i < goodies.length; i++){
      var entry = ({
    ...
    await fridgeCollection.doc(groupID).collection("goodies").doc(category).update({
      "goodies": fridgeProducts
    });
  }
  ...
  Future getGoodiesPerCategory(String category) async {
    ...
    String groupID = await getCurrentGroupID();
    await fridgeCollection.doc(groupID).collection("goodies").doc(category).get().then((value){
      data = value.data()["goodies"];
    });
    if(data != null){
      data.forEach((element) {
        products.add(new FridgeProduct(title: element["name"], number: element["number"].toDouble(), unit: element["unit"], category: element["category"]));
      });
    }
    return products;
  }
  ...
  Future<bool> addProductToList(Product product) async {
    String groupID = await getCurrentGroupID();
    var doc = await fridgeCollection.doc(groupID).collection("products").doc(product.title).get();

    if(doc == null || !doc.exists){
      await fridgeCollection.doc(groupID).collection("products").doc(product.title).set({
        'name': product.title,
        'category': product.category,
        'number': product.number,
        'unit': product. unit,
  ...
  Future saveNewRecipe(String recipe_name, String category, int portions, List ingredients, String instruction, String recipe_pic) async {
    String groupID = await getCurrentGroupID();
  ...
  Future getRecipesFromCategory(String category) async {
    ...
    await getCurrentGroupID().then((groupID) async {
      QuerySnapshot querySnapshot = await recipeCollection.doc(groupID).collection(category).get();
  ...
  Future leaveGroup(String groupID) async {
    ...
    await userCollection.doc(uid).update({
      'groups': FieldValue.arrayRemove(groupToDelete)
    });
    // remove user inside groups document
    await groupCollection.doc(groupID).update({
      'members': FieldValue.arrayRemove(userToDelete)
    });
  ...
  Future getSingleGroupInfo(String groupID) async{
    var groupInfo;
    await Future.wait([groupCollection.doc(groupID).get().then((doc){
      groupInfo = doc.data();
    })]);
    return groupInfo;
  }
  ...
  Future getAllGroupInfo() async {
    ...
    await Future.forEach(groups, (element) async {
      await getSingleGroupInfo(element).then((value) => groupInfo.add(value));
    });
  ...
  Future getCurrentGroupID() async {
    String uid = auth.getCurrentUser();
    var groupID;
  ...
  Future uploadGroupImageToFirebase(BuildContext context, File _imageFile) async {
  ...
  Future uploadRecipeImageToFirebase(BuildContext context, File _imageFile) async {
    String fileName = basename(_imageFile.path);

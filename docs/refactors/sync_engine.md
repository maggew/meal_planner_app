...
10. Delete old realtime tests; add coordinator integration test. *(Old `test/repositories/offline_first_*_repository_test.dart` and `test/services/shopping_list_sync_service_test.dart` deleted with step 9. Coordinator integration test added at `test/data/sync/sync_coordinator_test.dart` — uses the real `SyncEngine` + an in-memory `SyncMetaStore` + counting fake adapters, drives time with `package:fake_async`, and exercises page-open polling, `updateMealPlanMonth`, connectivity restore, lifecycle resume, `syncAll` failure isolation, and start/stop idempotency. To enable injection of fake adapters, `SyncCoordinator` now takes `SyncAdapter` (interface) instead of the concrete `MealPlanSyncAdapter`/`ShoppingListSyncAdapter` — pure typing change, no runtime impact.)*
